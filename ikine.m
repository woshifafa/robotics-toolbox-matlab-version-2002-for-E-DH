...
%        q= [0 pi/2  pi/2 0  0  0 ]';
	...
	tcount = 0;
	if ishomog(tr),		% single xform case 当tr为一个4*4矩阵时
		nm = 1;
		count = 0; %最终希望获得n个关节最后的角度
		while nm > stol,
		...
		for i=1:np
			...
			count = 0;
			while nm > stol,
                %e表示当前末端位姿与目标相差 (e(1),e(2),e(3)表示位移，e(4)-e(6)表示rpy绕三轴旋转角度)
				e = tr2diff(fkine(robot, q'), T) .* m;
				dq = pinv( jacob0(robot, q) ) * e ;% X = PINV(A) produces a matrix X of the same dimensions as A' so that A*X*A = A, X*A*X = X and A*X and X*A are Hermitian.
				q = q + dq;
				nm = norm(dq);
				count = count+1;
				if count > ilimit,
