...
function l = alink(dh, convention)
	...
	elseif length(dh) <= 7,
		% legacy DH matrix
		% alink([alpha A theta D sigma])

		if length(dh) < 5,
				error('must provide <alpha A dels theta D> params');
		end
		l.alpha = dh(1);
		l.A = dh(2);
        l.dels=dh(3);
		...
		if nargin > 1
			if strncmp(convention, 'new', 3) == 1,
				l.mdh = 1;
			elseif strncmp(convention, 'sta', 3) == 1,
				l.mdh = 0;
			else
				error('convention must be modified or standard or new ');
			end
		else
				l.mdh = 1;	% default to modified D&H
		end
 		l.offset = 0;
		if length(dh) >= 7,
			l.offset = dh(7);
