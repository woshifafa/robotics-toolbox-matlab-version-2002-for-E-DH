...
function tau = rne_mixdh(robot, a1, a2, a3, a4, a5)
	...
	%
		for j=1:n,
            ...
            if link.mdh==1    
                  ...
                  Pm(:,j) = [link.A; D*sin(alpha); D*cos(alpha)];
            end
		...
		for j=n:-1:1,
            ...
            if link.mdh ==1,
                if link.RP == 'R',
                    % revolute
                    tau(p,j) = nn'*z0 + ...
                        link.G^2 * link.Jm*qdd(j); %+link.G * friction(link, qd(j));
