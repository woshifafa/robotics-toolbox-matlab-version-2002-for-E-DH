...
function rnew = plot(robot, tg, varargin)
	...
	%
	np = numrows(tg); %获知tg行数
	n = robot.n

	if numcols(tg) ~= n,% 判断列数是否一致
	...
	if isempty(fh)
		...
		figure
		h = create_new_robot(robot, opt); %生成新robot
		...
		numchildren = length( get(gcf, 'Children') );%获取后代数量
...
function h = create_new_robot(robot, opt)
	...
	if opt.base,
        %disp('not hold')
		b = transl(robot.base);% 提取robot的XYZ
		line('xdata', [b(1);b(1)], ...
			'ydata', [b(2);b(2)], ...
			'zdata', [h.zmin;b(3)], ...
			'LineWidth', 4, ...
	...
	if opt.joints == 1,
		...
		for i=1:robot.n,
			...
			[xc,yc,zc] = cylinder(opt.mag/4, N); %直径为 opt.mag/4的 N 棱柱
			zc(zc==0) = -opt.mag/3;
			zc(zc==1) = opt.mag/3;
...
function animate(robot, q)
    ...
    if sum(robot.mdh == 1) == 0,
        ...
        for j=1:n,
            ...
            D=L{j}.D;
            if  L{j}.mdh==0
                t = t *linktran(al, A, q(j) , D);
            else if L{j}.mdh==1
                t = t *mlinktran(al , A, q(j) , D);
                else if L{j}.mdh==2
                t = t *newlinktran(al, A, q(j) , D);
                    else t = t *anolinktran(al, A, q(j) , D);
                    end
                end
            end

    % 		t = t * L{j}(q(j),j);%调用link.subsref函数  link(q) 返回值为A矩阵
    %       t
	...
	t = t *robot.tool;% robot.tool是eye(4*4)
